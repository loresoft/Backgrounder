// Copyright (c) 2015-2023, App vNext Polly https://github.com/App-vNext/Polly
...
public static class RetryHelper
{
    ...
    private static TimeSpan GetRetryDelayCore(BackoffType type, bool jitter, int attempt, TimeSpan baseDelay, ref double state, Func<double> randomizer)
    {
        ...
        if (jitter)
        {
            return type switch
            {
                BackoffType.Constant => ApplyJitter(baseDelay, randomizer),
                BackoffType.Linear => ApplyJitter(TimeSpan.FromMilliseconds((attempt + 1) * baseDelay.TotalMilliseconds), randomizer),
                BackoffType.Exponential => DecorrelatedJitterBackoffV2(attempt, baseDelay, ref state, randomizer),
...
#if !NETCOREAPP
            BackoffType.Linear => TimeSpan.FromMilliseconds((attempt + 1) * baseDelay.TotalMilliseconds),
            BackoffType.Exponential => TimeSpan.FromMilliseconds(Math.Pow(ExponentialFactor, attempt) * baseDelay.TotalMilliseconds),
#else
            BackoffType.Linear => (attempt + 1) * baseDelay,
            BackoffType.Exponential => Math.Pow(ExponentialFactor, attempt) * baseDelay,
#endif
            _ => throw new ArgumentOutOfRangeException(nameof(type), type, "The retry backoff type is not supported.")
        };
    }
